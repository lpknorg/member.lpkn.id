...
class Tbl_menu_model extends CI_Model
{
    ...
    // datatables
    function json() {
        $this->datatables->select('id,label,icon,type,link,parent,sort');
        $this->datatables->from('tbl_menu');
        //add this line for join
        //$this->datatables->join('table2', 'tbl_menu.field = table2.field');
        //$this->datatables->add_column('action', anchor(site_url('tbl_menu/read/$1'),'Read')." | ".anchor(site_url('tbl_menu/update/$1'),'Update')." | ".anchor(site_url('tbl_menu/delete/$1'),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), 'id');
        $this->datatables->add_column('view_only', '<div class="btn-group btn-group-sm" role="group" aria-label="Small button group">
                                                    <button onclick="load_controler(\'tbl_menu/read/$1\');" class="btn btn-default" type="button"><i class="fa fa-eye"></i></button>
                                                    </div>', 
                                        'id'
                                    );
        $this->datatables->add_column('update_only', '<div class="btn-group btn-group-sm" role="group" aria-label="Small button group">
                                                    <button onclick="load_controler(\'tbl_menu/read/$1\');" class="btn btn-default" type="button"><i class="fa fa-eye"></i></button>
                                                    <button onclick="load_controler(\'tbl_menu/update/$1\');" class="btn btn-primary" type="button"><i class="fa fa-pencil-square-o"></i></button>
                                                    </div>', 
                                        'id'
                                    );
        $this->datatables->add_column('delete_only', '<div class="btn-group btn-group-sm" role="group" aria-label="Small button group">
                                                    <button onclick="load_controler(\'tbl_menu/read/$1\');" class="btn btn-default" type="button"><i class="fa fa-eye"></i></button>
                                                    <button onclick="if(confirm(\'Are you sure?\')) load_controler(\'tbl_menu/delete/$1\');" class="btn btn-danger" type="button"><i class="fa fa-trash"></i></button>
                                                    </div>', 
                                        'id'
                                    );
