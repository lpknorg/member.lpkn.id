...
class Auth extends CI_Controller
{
	...
	public function index()
	{
		...
		else if (!$this->ion_auth->is_admin() && !$this->ion_auth_acl->has_permission('menu_users')) // remove this elseif if you want to enable this for non-admins
		{
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
				...
				if ($this->ion_auth->in_group('members')){
					redirect(base_url('page/profile'), 'refresh');
				}elseif ($this->ion_auth->in_group('instruktur')){
					redirect(base_url('page/profile_instruktur'), 'refresh');
				}else{
	...
	{
		if (!$this->ion_auth->is_admin() && !$this->ion_auth_acl->has_permission('edit_user')){
            $this->res['success'] = false;
			...
			if ($activation){
				$this->res['success'] = true;
            	$this->res['msg'] = "User Is Activated";
			}else{
            	$this->res['success'] = false;
            	$this->res['msg'] = "Error Activated";
			}
		}
        echo json_encode($this->res);
	...
	public function deactivate($id = NULL)
	{
		if (!$this->ion_auth->is_admin() && !$this->ion_auth_acl->has_permission('edit_user')){
			...
			$update = $this->ion_auth->deactivate($id);
			if($update){
            	$this->res['success'] = true;
            	$this->res['msg'] = "User Deactivated";
			}else{
            	$this->res['success'] = false;
            	$this->res['msg'] = "Admin Cannot deactivated";
			}
		}
        echo json_encode($this->res);
	...
	public function create_user()
	{
		...
		// validate form input
		$this->form_validation->set_rules('username', $this->lang->line('create_user_validation_username_label'), 'trim|required');
		...
		if ($this->form_validation->run() === TRUE && $this->ion_auth->register($identity, $password, $email, $additional_data))
		{
			$id_user = $this->db->query("SELECT id FROM users WHERE email = '".$email."'")->row();
				if ($this->ion_auth->is_admin())
				{
					...
					$this->ion_auth->remove_from_group('', $id_user->id);
					...
					if (isset($groupData) && !empty($groupData))
					{
						...
						{
							$this->ion_auth->add_to_group($grp, $id_user->id);
						}
	...
	*/
	...
	public function register(){
		// $url_api = 'https://www.emsifa.com/api-wilayah-indonesia/api/provinces.json'
		...
		if($ceknik > 0){
			...
			if($cekEmail > 0)
			{
				...
				if($insert){
		            ...
		            {
		                $id_user = $this->db->query("SELECT id FROM users WHERE email = '".$email."'")->row();
		                // $this->ion_auth->add_to_group(6, $id_user->id);
	...
	public function regis_success($id){
		$user = $this->db->where('id',$id)->get('users')->row();
		$data = array('user' => $user, );
		$this->load->view('auth/regis_success', $data);
	}

	public function verifikasi_email($id, $code = FALSE)
	{
		$activation = FALSE;
