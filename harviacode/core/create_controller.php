...
$string .="\n\n    public function index()
    {
        if (!\$this->ion_auth->is_admin() && !\$this->ion_auth_acl->has_permission('menu_".$table_name."')){
...
$string .= "\n\t\t    );
    ...
    {
        if (!\$this->ion_auth->is_admin() && !\$this->ion_auth_acl->has_permission('create_".$table_name."')){
            show_error('You must be an administrators to view this page.');
        }else{
            \$data = array(
                'button' => 'Create',
...
$string .= "\n\t\t\t    );
    ...
    {
        if (!\$this->ion_auth->is_admin() && !\$this->ion_auth_acl->has_permission('edit_".$table_name."')){
            show_error('You must be an administrators to view this page.');
        }else{
            \$row = \$this->".$m."->get_by_id(\$id);
...
$string .= "\n\t\t\t    );
    ...
    {
        if (!\$this->ion_auth->is_admin() && !\$this->ion_auth_acl->has_permission('delete_".$table_name."')){
            show_error('You must be an administrators to view this page.');
        }else{
            \$row = \$this->".$m."->get_by_id(\$id);
