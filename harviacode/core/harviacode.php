...
class Harviacode
{
    ...
    {
        // $this->sql->execute("INSERT INTO permissions (perm_key,perm_name) VALUE ('menu_".$table."')");
    ...
    function permissions_list($table)
    {
        $query = "INSERT INTO permissions (perm_key,perm_name) VALUEs (?,?)";
        $stmt = $this->sql->prepare($query) OR die("Error code :" . $this->sql->errno . " (not_primary_field)");
        $stmt->bind_param("ss", $key, $name);
        $key = 'menu_'.$table;
        $name = 'Menu '.label($table);
        $stmt->execute();
        $key = 'create_'.$table;
        $name = 'Create '.label($table);
        $stmt->execute();
        $key = 'edit_'.$table;
        $name = 'Update '.label($table);
        $stmt->execute();
        $key = 'delete_'.$table;
        $name = 'Delete '.label($table);
        $stmt->execute();
        $stmt->close();
        // $this->sql->close();
    }
